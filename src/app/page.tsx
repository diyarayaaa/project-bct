...
export default function DashboardPage() {
  ...
  // Paginated tickets for smooth rendering
  const totalPages = useMemo(() => {
    if (pageSize === 0) return 1;
    return Math.max(1, Math.ceil(sortedTickets.length / pageSize));
  }, [sortedTickets.length, pageSize]);

  const paginatedTickets = useMemo(() => {
    if (pageSize === 0) return sortedTickets;
  ...
  const handleExecuteBulkDelete = async () => {
    ...
    try {
      ...
      if (!res.ok || data.error) {
        setBulkDeleteError(data.error || 'Gagal menghapus tiket yang dipilih');
        return;
      }
      ...
      setBulkDeleteError((err as Error).message || 'Terjadi kesalahan sistem');
    } finally {
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 sm:gap-4">
        ...
        <MetricCard
          ...
          isActive={activeTab === 'service_on_progress'}
        />
        ...
        <MetricCard
          ...
          isActive={activeTab === 'barang_belum_diambil'}
        />
      ...
      {/* Sticky Fast Navigation Strip (GPU Accelerated, No Backdrop Blur lag) */}
      <div className="sticky top-[68px] sm:top-[76px] z-10 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xs p-2 sm:p-2.5 transition-colors">
        <div className="w-full flex items-center justify-between gap-2 sm:gap-3">
          ...
          {/* Center: Slim Metric Pills for 8 categories */}
            ...
            <button
              ...
              className={`inline-flex items-center gap-1 px-2 sm:px-2.5 py-1 rounded-lg text-xs font-bold transition-all shrink-0 cursor-pointer ${
                activeTab === 'masuk_hari_ini'
                  ...
                  : 'bg-sky-50 dark:bg-sky-950/60 text-sky-700 dark:text-sky-300 border border-sky-200/80 dark:border-sky-800/80 hover:bg-sky-100 dark:hover:bg-sky-900/60'
              }`}
            >
            ...
            <button
              ...
              className={`inline-flex items-center gap-1 px-2 sm:px-2.5 py-1 rounded-lg text-xs font-bold transition-all shrink-0 cursor-pointer ${
                activeTab === 'service_on_progress' || activeTab === 'on_progress'
                  ...
                  : 'bg-amber-50 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300 border border-amber-200/80 dark:border-amber-800/80 hover:bg-amber-100 dark:hover:bg-amber-900/60'
              }`}
            >
            ...
            <button
              ...
              className={`inline-flex items-center gap-1 px-2 sm:px-2.5 py-1 rounded-lg text-xs font-bold transition-all shrink-0 cursor-pointer ${
                activeTab === 'barang_di_vendor' || activeTab === 'waiting_vendor'
                  ...
                  : 'bg-purple-50 dark:bg-purple-950/60 text-purple-700 dark:text-purple-300 border border-purple-200/80 dark:border-purple-800/80 hover:bg-purple-100 dark:hover:bg-purple-900/60'
              }`}
            >
            ...
            <button
              ...
              className={`inline-flex items-center gap-1 px-2 sm:px-2.5 py-1 rounded-lg text-xs font-bold transition-all shrink-0 cursor-pointer ${
                activeTab === 'garansi_belum_dikirim'
                  ...
                  : 'bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-200/80 dark:border-rose-800/80 hover:bg-rose-100 dark:hover:bg-rose-900/60'
              }`}
            >
            ...
            <button
              ...
              className={`inline-flex items-center gap-1 px-2 sm:px-2.5 py-1 rounded-lg text-xs font-bold transition-all shrink-0 cursor-pointer ${
                activeTab === 'garansi_selesai'
                  ...
                  : 'bg-teal-50 dark:bg-teal-950/60 text-teal-700 dark:text-teal-300 border border-teal-200/80 dark:border-teal-800/80 hover:bg-teal-100 dark:hover:bg-teal-900/60'
              }`}
            >
            ...
            <button
              ...
              className={`inline-flex items-center gap-1 px-2 sm:px-2.5 py-1 rounded-lg text-xs font-bold transition-all shrink-0 cursor-pointer ${
                activeTab === 'service_selesai'
                  ...
                  : 'bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 border border-blue-200/80 dark:border-blue-800/80 hover:bg-blue-100 dark:hover:bg-blue-900/60'
              }`}
            >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs overflow-hidden transition-colors">
        ...
        {activeTab !== 'all' && (
          <div className="px-4 py-2.5 bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
            <span className="text-xs font-semibold text-slate-600 dark:text-slate-300">
              Filter aktif: <strong className="text-slate-900 dark:text-white capitalize">{activeTab.replace(/_/g, ' ')}</strong> ({sortedTickets.length} tiket)
            </span>
            <button
              type="button"
        ...
        {sortedTickets.length > 0 && (
          <div className="p-3 sm:p-4 bg-slate-50/80 dark:bg-slate-800/40 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-600 dark:text-slate-400">
            <div className="flex items-center gap-3">
              <span>
                ...
                -{' '}
                <strong className="text-slate-900 dark:text-white">
                  {pageSize === 0 ? sortedTickets.length : Math.min(currentPage * pageSize, sortedTickets.length)}
                </strong>{' '}
                dari{' '}
                <strong className="text-slate-900 dark:text-white">{sortedTickets.length}</strong> tiket
              </span>
              ...
              <div className="flex items-center gap-1.5 pl-2 border-l border-slate-300 dark:border-slate-700">
                ...
                <select
                  ...
                  className="px-2 py-1 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-800 dark:text-slate-200 text-xs font-semibold focus:outline-hidden"
                >
            ...
            {pageSize > 0 && totalPages > 1 && (
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="px-2.5 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 font-bold hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1 transition-colors"
                >
